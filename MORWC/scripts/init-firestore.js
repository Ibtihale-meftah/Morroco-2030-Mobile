// Script pour initialiser Firestore avec les données des villes et monuments
// À exécuter une seule fois pour migrer les données locales vers Firestore
// Usage: node --experimental-modules scripts/init-firestore.js
// OU mieux: npm install -g tsx && tsx scripts/init-firestore.ts
  ...
  try {
    ...
    for (const city of cities) {
      try {
        ...
        console.log(`✓ Ville ajoutée: ${city.name}`);
      } catch (error) {
    ...
    console.log("Ajout des monuments...");
    for (const monument of monuments) {
      try {
        await addDoc(collection(db, "monuments"), monument);
        console.log(`✓ Monument ajouté: ${monument.name}`);
      } catch (error) {
        console.error(`✗ Erreur pour ${monument.name}:`, error);
      }
    }
